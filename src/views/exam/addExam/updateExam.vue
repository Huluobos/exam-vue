<template>
  <div class="app-container">
    <el-form ref="addExamForm" :model="examForm" :rules="rules" label-width="90px" label-position="left">
      ...
      <el-row>
        <el-col :span="6">
          <el-form-item label="学段" prop="period">
            <el-select v-if="allSubStatus" disabled style="width:100%" v-model="examForm.period" placeholder="学段"
                       @change="changePeriod()" clearable>
              <el-option
            ...
            <el-select v-else-if="onlySubStatus" disabled style="width:100%" v-model="examForm.period"
                       placeholder="学段" @change="changePeriod()" clearable>
              <el-option
                  v-for="pre in examPeriodList"
                  :key="pre.value"
                  :label="pre.label"
            ...
            <el-select v-else style="width:100%" v-model="examForm.period" placeholder="学段" @change="changePeriod()"
                       clearable>
              <el-option
                  v-for="pre in examPeriodList"
                  :key="pre.value"
      ...
      <el-row>
        <el-col :span="6">
          <el-form-item label="年级" prop="grade">
            <el-select v-if="allSubStatus" disabled style="width:100%" v-model="examForm.grade" @change="changeGrade"
            ...
            <el-select v-else-if="onlySubStatus" disabled style="width:100%" v-model="examForm.grade"
                       @change="changeGrade" placeholder="年级" clearable>
              <el-option
                  v-for="gra in examGradeList"
                  :key="gra.value"
                  :label="gra.label"
            ...
            <el-select v-else style="width:100%" v-model="examForm.grade" @change="changeGrade" placeholder="年级"
                       clearable>
              <el-option
                  v-for="gra in examGradeList"
                  :key="gra.value"
      ...
      <el-row class="tal">
        <el-col class="bl-row" :offset="1" :span="23">
          <el-form-item class="textSmall" label="考试科目" prop="subNames">
            <el-row>
              <el-checkbox v-if="allSubStatus" disabled :indeterminate="isIndeterSubject" v-model="checkAllSubject"
                           @change="handleCheckAllSubjectChange">全选
              </el-checkbox>
              <el-checkbox v-else-if="onlySubStatus" disabled :indeterminate="isIndeterSubject"
                           v-model="checkAllSubject" @change="handleCheckAllSubjectChange">全选
              </el-checkbox>
              <el-checkbox v-else :indeterminate="isIndeterSubject" v-model="checkAllSubject"
                           @change="handleCheckAllSubjectChange">全选
              </el-checkbox>
            </el-row>
            <el-row>
              <el-checkbox-group v-if="allSubStatus" disabled v-model="examForm.subNames"
                ...
                <el-button disabled type="text" style="margin-left: 15px;" @click="addMoreSub">+添加自定义科目
                </el-button>
              </el-checkbox-group>

              <el-checkbox-group v-else-if="onlySubStatus" v-model="examForm.subNames"
                                 @change="handleCheckedCitiesSubjectChange">
                <el-checkbox v-for="subject in subjects" :label="subject" :key="subject"
                             :disabled="(subCheck && subCheck.indexOf(subject) > -1)">{{ subject }}
                </el-checkbox>
          ...
          <el-form-item class="textSmall" label="参考班级" prop="classIds">
            <el-row>
              <el-checkbox v-if="allSubStatus" disabled :indeterminate="isIndeterClass" v-model="checkAllClass"
                           @change="handleCheckAllclassChange">全选
              </el-checkbox>
              <el-checkbox v-else-if="onlySubStatus" disabled :indeterminate="isIndeterClass" v-model="checkAllClass"
                           @change="handleCheckAllclassChange">全选
              </el-checkbox>
              <el-checkbox v-else :indeterminate="isIndeterClass" v-model="checkAllClass"
                           @change="handleCheckAllclassChange">全选
              </el-checkbox>
            </el-row>
            <el-row>
              <el-checkbox-group v-if="allSubStatus" disabled v-model="examForm.classIds"
                                 @change="handleCheckedCitiesClassChange">
                <el-checkbox v-for="classes in classesList" :value="classes.id" :label="classes.id" :key="classes.id">
                  {{ classes.className }}
                </el-checkbox>
              </el-checkbox-group>
              <el-checkbox-group v-else-if="onlySubStatus" disabled v-model="examForm.classIds"
                                 @change="handleCheckedCitiesClassChange">
                <el-checkbox v-for="classes in classesList" :value="classes.id" :label="classes.id" :key="classes.id">
                  {{ classes.className }}
                </el-checkbox>
      ...
      <el-row class="tal">
        <el-col :span="24">
          <el-form-item prop="model" label="考试模式">
            <el-radio-group v-if="allSubStatus" disabled v-model.number="examForm.model">
              <el-radio :label="1" disabled>先扫后阅</el-radio>
              <el-radio :label="2" disabled>成绩导入</el-radio>
              <el-radio :label="3" disabled>小题分导入</el-radio>
            </el-radio-group>
            <el-radio-group v-else-if="onlySubStatus" disabled v-model.number="examForm.model">
              <el-radio :label="1" disabled>先扫后阅</el-radio>
              <el-radio :label="2" disabled>成绩导入</el-radio>
              <el-radio :label="3" disabled>小题分导入</el-radio>
            </el-radio-group>
            <el-radio-group v-else v-model.number="examForm.model">
              <el-radio :label="1" disabled>先扫后阅</el-radio>
              <el-radio :label="2" disabled>成绩导入</el-radio>
              <el-radio :label="3" disabled>小题分导入</el-radio>
            </el-radio-group>
      ...
      <el-row class="tal">
        <el-button :loading="loading" style="width: 95px;height: 32px;background: #ECF4FF;color: #202020" size="small" @click="goBack()">取消</el-button>
        <el-button :loading="loading" style="width: 100px;height: 32px;" type="primary" size="small" @click="handleUpdate()">保存</el-button>
      </el-row>
    </el-form>
  </div>
...
let {
  rules, examForm, loading, examTypeList, examPeriodList,
  examGradeList, examSemesterList, checkAllSubject, subjects, isIndeterSubject,
  checkAllClass, classesList, isIndeterClass, publishStatus
} = toRefs(data);
...
function handleCheckAllclassChange(val: any) {
  examForm.value.classIds = val ? classesList.value.map((val: any) => {
    return val.id
  }) : [];
...
function getExamById(examId: any) {
  getExamMessageById(examId).then((res: any) => {
    ...
    if (res.success) {
      ...
      examForm.value.model = res.data.model
      console.log('examForm: ', examForm.value);
...
function formatDateValue(now: any) {
  const year = dateZero(now.getFullYear()); //取得4位数的年份
  const month = dateZero(now.getMonth() + 1); //取得日期中的月份，其中0表示1月，11表示12月
  const date = dateZero(now.getDate()); //返回日期月份中的天数（1到31）
  const hour = dateZero(now.getHours()); //返回日期中的小时数（0到23）
  const minute = dateZero(now.getMinutes()); //返回日期中的分钟数（0到59）
  const second = dateZero(now.getSeconds()); //返回日期中的秒数（0到59）
