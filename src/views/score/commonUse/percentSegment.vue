<template>
  <div class="container">
    ...
    <div>
      <div class="title">百分比段对比</div>
      <div class="charts-class">
        <Echarts :options="options" :width="'100%'" :height="'450px'"></Echarts>
      </div>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  import {
    ...
    deleteSegmentApi, deleteAllShareSegmentApi, setDefaultSegmentApi, saveSegmentApi
  } from '@/api/analysis'
  ...
  function getAllSubjects() {
    getAllSubjectsApi(examId).then((res) => {
      ...
      if (subjects.value.length > 0) {
        ...
        curSubType.value = subjects.value[0].curSubType
  ...
  function setRadarData(data:any) {
    ...
    if(xDataArr.length > 10){
      dataSize = (xDataArr.length /10);
    }
    var dateZoomStr = [];
    dateZoomStr.push({
      type: 'inside',
...
//                    rotate:-30//-30度角倾斜显示
  ...
  function saveSegment() {
    ...
    for (let i=1;i<settingForm.settings.length;i++) {
      ...
      if (Number(preEndScore) >= Number(curEndScore)) {
        ElMessage.error({
          message:"结束值“" + curEndScore + "”不能小于前一个结束值“" + preEndScore + "”！<br/>注意：百分比段的指标请按升序输入！",
  ...
  function exportScoreSegmentResult() {
    ...
    document.location.href = VITE_GLOB_API_URL+"/newexam/score/exportScoreSegmentResult/"+examId+"?level=" + level
      + "&normType=2&scoreType=" + scoreType.value + "&classType=" + classType.value + "&exStud=true"
  }
